<template>
	<view class="template-mine tn-safe-area-inset-bottom">
		<!-- 顶部自定义导航 -->
		<!-- <tn-nav-bar :isBack="false" :bottomShadow="false" backgroundColor="none"></tn-nav-bar> -->
					...
					<view class="tn-flex tn-flex-col-center tn-flex-row-left" @click="tn('/minePages/set')">
						<view class="logo-pic tn-shadow">
							<view class="logo-image">
								<view v-if="!APP_USER" class="tn-shadow-blur" style="background-image:url('/static/user/no_login.png');width: 110rpx;height: 110rpx;background-size: cover;"></view>
								<view v-else class="tn-shadow-blur" :style="'background-image:url('+APP_USER.avatarUrl+');width: 110rpx;height: 110rpx;background-size: cover;'"></view>
							</view>
						</view>
						<view class="tn-padding-right">
							<view class="tn-padding-right tn-padding-left-sm tn-text-xl tn-text-bold">
								<text class="tn-color-brown--dark" v-if="!APP_USER" @click="login">点击登录</text>
								<text class="tn-color-brown--dark" v-else @click="logout">{{APP_USER.nickName}}</text>
							</view>
...
<script>
	import {Login,Test} from '@/libs/utils/http/api'
	export default {
		name: 'Mine',
		data() {
			return {}
		...
		methods: {
			logout() {
				console.log("退出登录")
				this.$t.vuex('APP_USER', null)
				this.$t.vuex('APP_TOKEN', null)
			},
			login() {
				var that = this;
				Login(that,{openid:'071RlUFa11U7NE01lGHa1EO97N3RlUFu'});
			},
			homeOnShow() {
				if (this.APP_USER) {
					console.log("存在用户信息")
